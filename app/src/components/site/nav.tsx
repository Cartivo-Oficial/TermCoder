  ...
  return (
    <header className="sticky top-0 z-40 border-b border-border bg-background/80 backdrop-blur-xl">
      {/* Three zones: mark hard left, links centred in the viewport, controls
          ...
          page rather than on whatever space the other two leave behind. */}
      <div className="relative mx-auto flex h-16 max-w-[1120px] items-center px-6">
        ...
        <nav className="absolute left-1/2 hidden -translate-x-1/2 items-center gap-7 text-[14px] text-muted-foreground md:flex">
          {LINKS.map((n) => (
            <a
              key={n}
              href={`${n}.html`}
