...
// Validates site/dist, the Astro build output — run after `npm run build`.
const toolsDir = dirname(fileURLToPath(import.meta.url));
const site = join(dirname(toolsDir), "dist");
const repoRoot = dirname(dirname(toolsDir));

// callback.html is a legacy static file carried over into public/, not an
// Astro-built page — it gets the "required asset" existence check below,
// not the full per-page content checks (see REQUIRED_ASSETS).
const CALLBACK = "callback.html";
...
// Astro emits its own inline <style>astro-island,...</style> tag on every
// page that hydrates a client island — that's framework output, not
// authored markup, so unlike the legacy site guard we do NOT fail on
// inline <style> blocks here.

// Astro bundles CSS with a hashed filename (e.g. /_astro/global.HASH.css)
// instead of a fixed style.css, so instead of checking for a literal
// style.css link, assert each page links a bundled stylesheet.
const STYLESHEET = /<link[^>]+rel="stylesheet"[^>]+href="[^"]*_astro\/[^"]+\.css[^"]*"/i;

for (const page of pages) {
  const html = readFileSync(join(site, page), "utf8");

  if (!STYLESHEET.test(html)) fail(`${page}: does not link a bundled stylesheet (_astro/*.css)`);
  ...
  // changelog.html legitimately lists historical version numbers as plain
  // text (that's the point of a changelog); every other page must link
  // releases/latest instead of pinning a version.
...
const REQUIRED_ASSETS = ["favicon.png", "app.png", "hero-session.js", "config.js", "auth.js", CALLBACK];
...
const changelog = readPage("changelog.html");
if (changelog) {
  const changelogMd = readFileSync(join(repoRoot, "CHANGELOG.md"), "utf8");
  const latest = changelogMd.match(/^##\s+(\d+\.\d+\.\d+)/m);
  if (!latest) {
    fail("CHANGELOG.md: could not find a top-level version heading (## x.y.z)");
  } else if (!changelog.includes(latest[1])) {
    fail(`changelog.html: does not mention the latest version (${latest[1]})`);
  }
}
